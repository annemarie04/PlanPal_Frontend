...
function DailyViewPage() {
    ...
    const getByDay = (day: string) => {
        if (taskList == null || activityList == null)
            return [];
        ...
        return filteredList.sort((activ1: Task | Activity, activ2: Task | Activity) => {
    ...
    return (
        <div className={styles.wholePage}>
            ...
            <div className={styles.pageContainer}>
                <div className={styles.titluDaily}> <h1> Daily view </h1> </div>
                <div className={styles.daySelectBar}>
                    Bara de selectare a zilei
                </div>
                    ...
                    <div className={styles.activitiesContainer}>
                        {finalList.map((activity) => (
                            <>
                                {
                                    activity.status === "done" ? null : (
                                        <Link href={activity.date ? `/tasks/view/${activity._id}` : `/activities/view/${activity._id}`}>
                                            <div key={activity._id} className={styles.timelineItem}>
                                                ...
                                                <div className={styles.dateContainer}>
                                                    {
                                                        toDDMMYYYY(activity.date ? activity.date : activity.start_date)
                                                    },
                                                    {
                                                        getFormattedTime(activity.date ? activity.date : activity.start_date)
                                                    }
                                                </div>
                                            </div>
                                        </Link>
